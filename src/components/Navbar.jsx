    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-[#050812]/88 shadow-[0_8px_34px_rgba(0,0,0,0.28)] backdrop-blur-xl">
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-1 md:flex">
          {navItems.map((item) => (
            <a key={item.href} href={item.href} className="rounded-lg px-3 py-2 font-mono text-xs text-slate-300 transition hover:bg-white/[0.08] hover:text-white">
      ...
      {open && (
        <div className="border-t border-white/10 bg-[#050812]/96 px-4 py-4 backdrop-blur md:hidden">
          <div className="mx-auto grid max-w-7xl gap-2">
            {navItems.map((item) => (
              <a key={item.href} href={item.href} onClick={() => setOpen(false)} className="rounded-lg border border-white/10 bg-white/[0.03] px-4 py-3 font-mono text-sm text-slate-200">
                ./ {item.label.toLowerCase()}
              </a>
            ))}
          </div>
