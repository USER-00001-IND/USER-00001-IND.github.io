  ...
  return (
    <section id="contact" className="section-shell">
      <div className="mx-auto max-w-7xl">
        <SectionHeading eyebrow="Contact" title="Start a conversation or review the code trail." command="ssh recruiter@rajmohan.dev" />
        <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="premium-card p-5">
            ...
            <div className="mt-6 space-y-3">
              ...
              <a href={`tel:${profile.phone.replace(/\s/g, "")}`} className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.035] p-4 text-slate-200 transition hover:border-emerald-300/40 hover:bg-emerald-300/[0.08]">
                <Phone className="text-emerald-300" size={20} /> {profile.phone}
              </a>
          ...
          >
            <div className="mb-5 flex items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-1 font-mono text-xs text-slate-500">$ compose --message</p>
              </div>
              <div className="hidden rounded-full border border-emerald-300/20 bg-emerald-300/[0.08] px-3 py-1 font-mono text-xs text-emerald-200 sm:block">online</div>
            </div>
            ...
            <label className="mt-4 grid gap-2 font-mono text-xs text-slate-400">
              Message
              <textarea rows="6" className={`${inputClass} resize-none`} placeholder="Write a short message..." />
            </label>
